...
```
📊 "各分群分别有多少人?有什么特征?"        → 调工具查分群统计,回答附图表
🔍 "高价值核心用户是谁?怎么定义的?"        → 展示业务命名 + 决策树划分规则
📈 "上个月哪个分群流失最严重?"             → 快照历史环比 + 趋势图
🚨 "有流失风险的高价值用户有哪些?"          → Watcher 用户级预警触发分析
🛍️ "帮我找一款适合预算 200 内的跑步耳机"    → 商城模式:搜商品 + 画像推荐 + 一键加购
📦 "用户画像分析报告给我来一份"             → 纯指令 Skill 编排四工具出综合分析
```

每个问题都会在界面实时展示执行链路:意图识别 → 模型决策 → 工具调用 → 数据校验 → 事实核查(SSE 逐步渲染);回答里的数字都经过三层规则核查,与真实数据不符会被拦截或标注警告。
...
三件套）。国内拉镜像:compose 按服务设前缀变量(不同镜像可用源不同,实测 etcd 走
`quay.m.daocloud.io/`、milvusdb/milvus 与 minio 走 `hub.rat.dev/` 或
`docker.1ms.run/`,大镜像多源层缓存可合流续拉):
`ETCD_IMAGE_PREFIX=quay.m.daocloud.io/ MILVUS_IMAGE_PREFIX=hub.rat.dev/ MINIO_IMAGE_PREFIX=hub.rat.dev/ docker compose -f docker-compose.milvus.yml up -d`;
直连能通时无需任何前缀。
两种后端挂掉都自动回退内存,接口不变。
...
- 代码质量:**351 项测试**(337 pytest + 14 vitest,纯函数 / Agent 图剧本化 / API 集成 / 前端,全部离线秒级)+ CI 每 push 自动执行
...
├── log/            # JSON 结构化日志
├── tests/          # 351 项测试(337 pytest + 14 vitest:纯函数 + Agent 图 + API 集成 + 前端)
