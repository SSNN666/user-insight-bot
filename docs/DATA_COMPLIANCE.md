...
> ⏱️ **时间线平移**:原始数据为 2016 年历史数据。加载时自动平移到当前
> 时间线(数据最新日期 = 今天 - 1 天),与商城运行时订单(真实系统时间)
> 同处一个 RFM 基准,保证"最近 N 天"口径一致、下单即入分析。相对结构
> (recency/频次/金额分布)在平移中保持不变。
...
- JData 无商品名字段、分类/品牌为数字编码 → 按品类码词库确定性合成中文可检索
  商品名(品类码 4-11 → 中文品类 + 品名词,如 "耳机-9001"/"影音数码";与价格
  合成同级演示口径,同源覆盖商城搜索/品类筛选/漏斗分组;合成词仅作检索与展示,
  不代表真实商品名;推荐 Skill 品类不匹配时自动回退热门商品);
