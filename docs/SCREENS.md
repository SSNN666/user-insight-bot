...
- 输入购物问句:「有没有 200 元内的降噪耳机推荐?」——贴合页面人设
  (placeholder 是"输入商品名或购物需求";该页 mode=auto,意图路由到购物
  场景,步骤条会真实走过 search_products 工具调用)
- 在回答流式输出**进行中**截屏:右侧/下方应有步骤条
  (意图识别 → 模型决策 → 工具调用 → 数据校验 → 事实核查),带进度感;
  若想连商品卡片+一键加购一起入镜,等回答结束再补一张同页截图

> 注:导购页 mode=auto,问分析问题(如"各分群多少人")也会被意图路由切到
> 分析工具并正常回答——技术可行,但主截图建议贴合页面人设;想展示"同一
> 入口意图分流",可以连续问购物+分析两题,步骤条的意图标签会自己切换。
